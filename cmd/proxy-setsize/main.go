...
func main() {
	...
	if len(flag.Args()) < 1 {
		glog.Exitf("error: no desired size given")
	}
	desiredSize, err := strconv.Atoi(flag.Args()[0])
	if err != nil {
		glog.Exitf("illegal desired size: %s", err)
	}
	...
	glog.Infof("setting desired size to %d", desiredSize)
	err = poolProxy.SetDesiredSize(desiredSize)
	if err != nil {
